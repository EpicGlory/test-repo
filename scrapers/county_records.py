"""Scraper for Maricopa County Assessor API and building permit records."""
...
class CountyRecordsScraper(BaseScraper):
    """Scrapes Maricopa County Assessor API and permit records.
    ...
    def __init__(self, config: dict):
        ...
        self.assessor_api = county_cfg.get("assessor_api", "https://mcassessor.maricopa.gov/api")
        self.permit_viewer_url = county_cfg.get(
            "permit_viewer", "https://apps.pnd.maricopa.gov/PermitViewer"
        )
        self.accela_url = county_cfg.get(
            "accela_portal", "https://accela.maricopa.gov/CitizenAccessMCOSS"
        )
        self.target_zips = []
        for loc in config.get("search", {}).get("locations", []):
            self.target_zips.extend(loc.get("zip_codes", []))
    ...
    def scrape(self) -> List[Property]:
        ...
        logger.info("Searching Maricopa County permit records...")
    ...
    def _lookup_address(self, address: str) -> Optional[dict]:
        """Look up a single address via the Maricopa County Assessor API."""
        try:
            # The Assessor API accepts address searches
            search_url = f"{self.assessor_api}/v1/search"
            params = {
                "address": address,
                "type": "address",
            }
    ...
    def _lookup_address_alternative(self, address: str) -> Optional[dict]:
        """Try alternative lookup via the Assessor website."""
        try:
            search_url = f"https://mcassessor.maricopa.gov/mcs.php"
            params = {"q": address}
    ...
    def _search_permit_viewer(self) -> List[Property]:
        """Search the Maricopa County Permit Viewer for new construction permits."""
        properties = []
    ...
    def _parse_permit_entry(self, entry) -> Optional[Property]:
        ...
        if not any(z in text for z in self.target_zips):
            ...
            if not any(city in text_lower for city in ["scottsdale", "paradise valley"]):
                return None
        ...
        prop = Property(
            ...
            source="Maricopa County Permits",
    ...
    def _lookup_permit(self, address: str) -> Optional[dict]:
        ...
        try:
            search_url = f"{self.accela_url}/Cap/CapHome.aspx"
            self._delay()
            response = self.session.get(
                search_url,
                headers=self._get_headers(),
