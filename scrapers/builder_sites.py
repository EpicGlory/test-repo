"""Scraper for luxury home builder websites in Scottsdale/Paradise Valley."""
...
class BuilderSitesScraper(BaseScraper):
    ...
    def _find_project_links(self, soup: BeautifulSoup, base_url: str) -> List[str]:
        ...
        for a_tag in soup.find_all("a", href=True):
            ...
            is_scottsdale_pv = any(
                loc in href_lower or loc in text
                for loc in ["scottsdale", "paradise", "silverleaf", "estancia", "desert mountain", "dc ranch"]
            )

            if is_project_link or is_scottsdale_pv:
    ...
    def _parse_card(self, card: Tag, builder: dict, url: str) -> Optional[Property]:
        ...
        if price:
            ...
            if price_num and price_num < 5_000_000:
                return None
    ...
    def _extract_address(self, text: str) -> str:
        """Extract an Arizona-style address from text."""
        patterns = [
            # Standard address: 12345 N Example St, Scottsdale
            r"(\d{1,6}\s+(?:[NSEW]\.?\s+)?[\w\s]+(?:St|Street|Ave|Avenue|Blvd|Boulevard|Dr|Drive|"
            r"Ct|Court|Ln|Lane|Rd|Road|Way|Trail|Trl|Path|Pl|Place))\s*,?\s*"
            r"(?:Scottsdale|Paradise Valley)",
            # Lot-style: Lot 5, Silverleaf
            r"(Lot\s+\d+\s*,?\s*[\w\s]+)",
        ]
    ...
    def _extract_style(self, text: str) -> str:
        ...
        styles = [
            ...
            "Transitional", "Ranch", "Minimalist", "Mid-Century Modern",
            "Southwestern", "Adobe", "Hacienda", "Craftsman",
        ]
        text_lower = text.lower()
        for style in styles:
    ...
    def _is_in_target_area(self, prop: Property) -> bool:
        """Check if property is in the target Scottsdale/PV area."""
        if prop.city:
            return prop.city.lower() in ("scottsdale", "paradise valley")

        # Check address and details for location hints
        combined = f"{prop.address} {prop.details} {prop.community}".lower()
        return any(
            loc in combined
            for loc in ["scottsdale", "paradise valley", "85255", "85253", "85259", "85262", "85266", "85260"]
        )

    def _text_mentions_target_area(self, text: str) -> bool:
        """Check if text mentions the target area."""
        text_lower = text.lower()
        return any(
            loc in text_lower
            for loc in [
                ...
                "silverleaf", "desert mountain", "dc ranch", "estancia",
                "troon north", "whisper rock", "pinnacle peak",
            ]
        )
