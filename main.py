#!/usr/bin/env python3
"""
Luxury Homes Scraper - configurable target market.

Scrapes multiple sources for UHNWI luxury homes that are not yet
completed (design, pre-construction, construction phases). Target
market (cities, zip codes, communities, builders, county records)
is fully driven by config.yaml.
...
def process_properties(properties: list, config: dict, county_data: dict = None) -> list:
    ...
    properties = enrich_properties(properties, county_data, config=config)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Scrape luxury homes for a configurable target market"
    )
    parser.add_argument(
        "--source",
        nargs="+",
    ...
    # Process results
    properties = process_properties(properties, config)
    ...
    if properties:
        ...
        output_path = write_to_excel(
            ...
            filename_prefix=output_cfg.get("filename_prefix", "luxury_homes"),
